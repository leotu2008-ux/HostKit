 ...
 * configured, by judgement on top of that. This is the "find_venues" card's
 * destination from the agent panel (Milestone B): hidden, not broken, when
 * there's nothing real to show. Search is unconfigured, the event's city
 * isn't one HostKit geocodes, Apple errors, or Apple simply has nothing —
 * all four render the same quiet empty state rather than a 500 or a dead end.
 */
export default async function AgentVenuesPage({
  params,
}: PageProps<"/events/[id]/agent/venues">) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-xl text-ink">Find a venue</h2>
