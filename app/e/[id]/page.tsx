...
export async function generateMetadata({
  ...
  if (!event?.published) return { title: "Event" };
...
export default async function PublicEventPage({
  ...
  if (!event.published && !isOwner) notFound();
    ...
    <main className="flex flex-1 flex-col pb-[7.5rem]">
      ...
      <div className="px-5 pt-5">
        <p className="text-[12px] font-medium tracking-[0.06em] text-clay uppercase">
          {event.published ? "Public" : "Unlisted preview"} ·{" "}
        ...
        <dl className="mt-5 space-y-3 text-[15px]">
          ...
          <div>
            ...
            <dd className="mt-0.5 text-ink">{event.city}</dd>
          </div>
        </dl>
        ...
        <div className="mb-2 flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="truncate text-[12px] text-ink-mute">
              {formatEventWhen(event.date, event.durationHours)} · {event.city}
            </p>
          </div>
