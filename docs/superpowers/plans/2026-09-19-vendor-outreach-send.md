...
| `prisma/migrations/<generated>_inquiry_recipient/` | Create (generated) | One `ALTER TABLE … ADD COLUMN`. |
...
- Create: `prisma/migrations/<generated>_inquiry_recipient/migration.sql` (generated by the CLI — do not hand-write it)
...
(The `composeInquiry, normalizeRecipient` import was added in Task 1 — extend that line rather than adding a second one.)
...
All three must pass. CI additionally runs `npx prisma migrate deploy` against a throwaway Postgres and then Playwright (`.github/workflows/ci.yml`). **This plan adds a migration**, so the CI migrate step is load-bearing here in a way it is not for most changes — if it fails, the generated SQL is wrong, not the tests.
...
**1. Spec coverage.** The problem statement names outreach to *speakers, security, venues and catering*. Venues and catering are `ListingCategory` values and are covered. Speakers are reachable today through `CollaboratorKind`, which `composeInquiry` already writes questions for, so they inherit the send loop. **Security is not covered** — `ListingCategory` has no `SECURITY` value and adding one was explicitly not the chosen slice. That is a real gap against the stated problem, recorded here rather than quietly dropped; it is roughly a one-day change (enum value, migration, label, icon, template weights) and wants its own plan.
