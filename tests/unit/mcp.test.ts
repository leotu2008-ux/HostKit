...
vi.mock("@/lib/db", () => ({
  db: {
    mcpGrant: { findUnique: mocks.findGrant, updateMany: mocks.updateGrant },
    event: { findFirst: mocks.findEvent, findMany: mocks.listEvents },
  },
}));
vi.mock("@/lib/venues/search", () => ({ searchVenues: mocks.search, venueSearchProvider: mocks.provider }));
vi.mock("@/lib/rate-limit", () => ({
  assertRateLimit: mocks.limit,
  clientIp: () => "test",
  RateLimitError: class extends Error {},
}));
import { authorizationRequest, mcpConfig } from "@/lib/mcp/oauth-config";
...
const authParams = {
  ...
  scope: "events:read venues:search",
  state: "state",
};
const grant = () => ({
  id: "grant-1",
  ...
  revokedAt: null,
  sessionVersion: 2,
  user: { sessionVersion: 2, emailVerifiedAt: new Date() },
});
const tokenForm = () =>
  new URLSearchParams({
...
beforeEach(() => {
  ...
  vi.stubEnv(
    "MCP_CLIENTS_JSON",
    JSON.stringify([{ id: "claude", name: "Claude", redirectUris: [authParams.redirect_uri] }]),
  );
  mocks.findGrant.mockResolvedValue(grant());
  mocks.updateGrant.mockResolvedValue({ count: 1 });
  mocks.provider.mockReturnValue("google");
...
describe("OAuth boundaries", () => {
  it("accepts registered redirects and S256 only", () => {
    ...
    for (const bad of [
      { redirect_uri: "https://client.example/callback/other" },
      ...
      expect(() => authorizationRequest({ ...authParams, ...bad })).toThrow();
    }
  ...
  it("rejects wrong PKCE, redirect, client, resource and expired grants", async () => {
    for (const [key, value] of [
      ...
      form.set(key, value);
      await expect(exchangeToken(form)).rejects.toThrow();
    }
  ...
  it("requires configured client credentials", async () => {
    vi.stubEnv(
      "MCP_CLIENTS_JSON",
      JSON.stringify([{ id: "claude", name: "Claude", secret: "s".repeat(32), redirectUris: [authParams.redirect_uri] }]),
    );
    await expect(exchangeToken(tokenForm())).rejects.toMatchObject({ code: "invalid_client" });
    await expect(exchangeToken(tokenForm(), { id: "claude", secret: "s".repeat(32) })).resolves.toHaveProperty(
      "access_token",
    );
  });

  it("rotates refresh tokens with a compare-and-swap", async () => {
    const form = new URLSearchParams({ grant_type: "refresh_token", client_id: "claude", refresh_token: code });
    await exchangeToken(form);
    expect(mocks.updateGrant.mock.calls[0][0].where.refreshHash).toBe(hashToken(code));
    mocks.updateGrant.mockResolvedValue({ count: 0 });
    await expect(exchangeToken(form)).rejects.toThrow();
  });

  it("rejects absent, native, expired, revoked and version-mismatched tokens", async () => {
    ...
    for (const change of [
      ...
      mocks.findGrant.mockResolvedValue({ ...grant(), ...change });
      expect(await authenticateMcp(req())).toBeNull();
    }
    mocks.findGrant.mockResolvedValue(grant());
    expect(await authenticateMcp(req())).toMatchObject({ userId: "user-1" });
  });
});

async function withClient(scopes: string[], run: (client: Client) => Promise<void>) {
  const server = createOAuthMcpServer({ userId: "user-1", scopes });
...
describe("OAuth MCP tools via official SDK client", () => {
  ...
  it("scopes list and brief queries to the token user and omits private fields", async () => {
    mocks.listEvents.mockResolvedValue([{ id: "one" }, { id: "two" }]);
    ...
    await withClient(["events:read"], async (client) => {
      ...
      const args = mocks.findEvent.mock.calls[0][0];
      expect(args.where).toEqual({ id: "someone-elses-event", ...eventAccess("user-1") });
      expect(args.select.claimToken).toBeUndefined();
      expect(args.select.guests).toBeUndefined();
    });
  });
  ...
  it("never searches for an inaccessible event", async () => {
    ...
    await withClient(["venues:search"], async (client) => {
      expect(
        (await client.callTool({ name: "search_venues", arguments: { eventId: "other", query: "private room" } }))
          .isError,
      ).toBe(true);
      expect(mocks.search).not.toHaveBeenCalled();
    });
  ...
  it("searches the event city and reports unverified availability", async () => {
    mocks.findEvent.mockResolvedValue({ city: "Boston, MA" });
    mocks.search.mockResolvedValue([{ id: "v1", name: "Venue" }]);
    await withClient(["venues:search"], async (client) => {
      const response = await client.callTool({
        name: "search_venues",
        arguments: { eventId: "mine", query: "private room" },
      });
      expect(mocks.search).toHaveBeenCalledWith("private room", "Boston, MA");
      expect(response.structuredContent).toMatchObject({
        provider: "google",
        verification: expect.stringContaining("not verified"),
      });
    });
  });
});
...
describe("HTTP transport", () => {
  ...
  it("returns a discovery challenge without authentication", async () => {
    const response = await POST(new Request(`${origin}/api/mcp`, { method: "POST", body: "{}" }));
  ...
  it("falls through to a bearer 401 when the token is not a live grant", async () => {
    process.env.AUTH_SECRET = "test-secret-for-mcp-oauth";
    mocks.findGrant.mockResolvedValue(null);
    const response = await POST(req({ jsonrpc: "2.0", id: 1, method: "tools/list" }));
    expect(response.status).toBe(401);
    expect(response.headers.get("WWW-Authenticate")).toContain("oauth-protected-resource");
    const body = (await response.json()) as { error: { message: string } };
    expect(body.error.message).toMatch(/bearer token/i);
    expect(JSON.stringify(body)).not.toMatch(/get_event_brief/);
  });
  ...
  it("handles a tool call without shared sessions", async () => {
    ...
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ result: { structuredContent: { events: [] } } });
  });
});
